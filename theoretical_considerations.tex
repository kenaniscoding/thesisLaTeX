...
\gls{ripeness}, and \gls{bruises}, along with a single mango for analysis. The Process
...
microcontroller, camera, and \acr{LED} lights).
...
\acr{KNN} to supplement the classification tasks, particularly for
...
microcontroller to control the Raspberry Pi camera, actuators, \acr{LED} lights, and
...
where \gls{not:real_world_dimension} is the real world dimension of the object,
 ...
 \gls{not:distance_from_cam_to_object} is the distance from the camera to the object,
  and \gls{not:focal_length} is the focal length of the camera. This relationship 
  follows from the pinhole camera model, where the real-world  
  dimension is proportional to the image dimension and the ratio
   of distance to focal length \cite{badali2005scale}.
