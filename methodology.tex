...
\subsection{Mango Position} \label{sec:mango_positioning}
In the image acquisition system, the mango is always positioned above the camera
and parallel to the metallic rollers and gap. This is so that the size
classification would be consistent for both image capturing attempts. Once the
mango has already been graded, the mango would exit the image acquisition system
parallel to the metallic rollers and parallel to the long conveyor belt. In the
case that the mango would go towards the small conveyor belt, it would be
perpendicular to the small conveyor belt.

\begin{figure}[!htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{/size_rev/position}
	\caption{conveyor positioning}
	\label{fig:mango_pos}
\end{figure}

Figure~\ref{fig:mango_pos} shows the position of the mango from the image
acquisition system which are the mangoes labeled 1 and 2.  When the mango is
already graded, it would be sorted using the T sorter seen on mangoes 3, 3.1,
and 3.2.
...
\begin{figure}[!htbp]
    ...
    \caption{Image Acquisition Dimensions}
...
Bayes, and k-Mean clustering and various Efficientnet models. This comparative
...
For the farm one of our members went to interview the head farmer (Jerry Bravante) as seen on Figure~\ref{fig:farm-of-mine}, 
it is located at Ibaan, Batangas. He as 50 years of experience being a farmer and 20 years of experience in quality standards 
of different mango fruits variations such as Carabao, Pico, Indian, and Apple. Additionally, the farm has a total of 4 hectares.
...
For the training, everything was done on a laptop, specifically the Acer Predator Helios 16 (PH16-71, 2023 model). The technical specifications of this unit are: Intel Core i9-13900HX processor, NVIDIA RTX 4070 GPU with 8GB VRAM, and 32GB DDR5 RAM running at 5600MHz.
...
bruised samples are included as seen in Figure~\ref{fig:ripeness_cnn_datasplit}. 
...
Likewise, The green would represent the underripe mangoes while the green yellow would represent the semi ripe while the yellow would
represent the ripe mangoes. 
In other words green is underripe, yellow is ripe, and yellow green is semi ripe mangoes.
As reference, Figure~\ref{fig:mangoRipeness} shows the different ripeness stages for Carabao/Pico mangoes \citet{doa2004}.
...
\subsubsection{Determining the Ranges for Mango Sizes Based on Area}
A total of 42 Carabao mangoes, 27 from Batch 1 and 15 from Batch 2, were collected to serve as the dataset for size classification.  Each mango will be manually measured using a caliper to obtain its length and width, ensuring consistent and accurate dimensional data. These measurements will then be used to compute the approximate area, which will serve as the primary feature for analysis. All recorded values will be compiled and converted into CSV format, allowing them to be used as a structured dataset for further statistical processing. The dataset will then be analyzed using two methods namely K-means clustering, an unsupervised technique that will be applied to identify natural groupings in the area values, and a quartile-based classifier, which will categorize mangoes based on their statistical distribution. Both approaches will be applied to determine the ideal ranges for the size categories, small, medium, and large.

\subsubsection{Estimating the Carabao Mango Size}
Mango size will be estimated through an image-processing workflow implemented in Python using OpenCV. Each mango image will first be converted from the BGR to HSV color space to facilitate segmentation based on characteristic fruit colors, namely green, yellow, and yellow-green. Binary masks will be generated for each color range and combined to isolate the mango region. Morphological operations such as opening and closing will then be applied to remove noise and refine the mask. The largest contour will be extracted to represent the mango, and a bounding rectangle will be fitted around it. To convert pixel dimensions into real-world measurements, a scaling factor will be established using a reference, the conveyor belt gap which has a fixed size that can be measured in cm and its corresponding pixel count in the image. The bounding box dimensions will be multiplied by this scaling factor to obtain mango length and width in centimeters. The estimated area will be computed as the product of these dimensions, and classification thresholds will be applied based on the optimal area ranges determined by statistical means. Each mango will be measured twice, once from the top face and once from the bottom face, which represent its largest visible areas. The two measurements will then be averaged to obtain a more reliable estimate of size. The conveyor system will fix the mango’s position during measurement, preventing slanting or unwanted orientations that could introduce error.

\subsubsection{K-Means Classification}
The K-Means clustering algorithm can be utilized to classify carabao mango data into three size categories of small, medium, and large by specifying the parameter n\_clusters = 3, which will pertain to the number of size classes. Prior to clustering, the area of each mango will be computed from its length and width measurements, producing a single feature that will represent overall fruit size. The input data will therefore consist of area values organized in a simple dataset format. After the algorithm runs, it will compute the coordinates of three cluster centers, where each center will represent the mean area of a group. The process will then assign a cluster index to each mango observation \citep{scikit-learn}. Because K-Means is an unsupervised method, these numerical cluster indices will need to be interpreted externally and assigned the labels of 'small,' 'medium,' and 'large' based on the physical dimensions represented by their respective cluster centers.

\subsubsection{Quantile-Based Classifier}
Quantile-based classification approach will also be employed to categorize carabao mangoes into three distinct size categories, namely small, medium, and large in order to determine the ideal range for sizes. From the length and width data, the area of each mango will be computed to serve as a single feature representing overall fruit size. This transformation will ensure that classification is based on a unified measure of size rather than separate dimensions.The quantile-based classifier will then be applied to the computed area values. The method will be generalized for three populations 
($\Pi_1$, $\Pi_2$, $\Pi_3$, representing g = 3 classes).
A new observation, defined by its computed area, will be assigned to the population that yields the lowest quantile distance, expressed as
$\Phi_k(z, \theta)$,
where k denotes the population index. The classifier will rely on determining the quantile functions 
$q_k(\theta)$ 
for each class distribution. A crucial step in this procedure will involve selecting the optimal quantile percentage 
$\theta$, which will minimize misclassification error in the training sample and define the empirically optimal quantile classifier. The median classifier will be considered as a special case of this rule, corresponding to 
$\theta = 0.5$
\citep{hennig_quantile-based_2013}. Since the method is not scale equivariant, variable scaling will be performed prior to classification to ensure comparability across observations.
...
of experience in mango species such as carabao, pico, indian, apple mango.
Joined with 2 other professionals in the same farm as Sir Bravante their
...
agreement, a substantial sample set was utilized. Each expert evaluated 26
