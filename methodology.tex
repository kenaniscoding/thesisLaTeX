...
% todo: Add the explanation that we trained and tested cnn, knn, naive bayes, etc
...
and each day the Carabao mangoes would be pictured until they are yellow ripe. Likewise, Figure~\ref{fig:boxes-mangoes} shows
the 8 kilogram green Carabao mangoes from the Bicol region. The same mangoes from Bicol are seen on the Figure~\ref{fig:table-mangoes}.
Note that the mangoes were individually captured one at a time at both cheek sides as a video format. 
% todo: ask francis on what software and method did he use to annotate the mangoes
\begin{figure}[!htbp]
    \centering
...
This means that if the mango has any of these
\begin{figure}[!htbp]
	\centering
...
Likewise, it usually appear as grey-brown or black rots starting from the stem end, often without obvious spores, that can spread rapidly into the mango. 

\subsubsection{Dendritic Spot}
They are small black spots with irregular edges scattered across the skin. 
Furthermore, it grow slowly and do not penetrate into the flesh, remaining largely superficial.

\subsubsection{Anthracnose}
It appears in two forms. First form is through body anthracnose. 
...
Likewise, the second form is stem end anthracnose. 
Stem end anthracnose occurs around the stem, also presenting as black rots.
While these rots do not penetrate deeply into the flesh, advanced cases may show pink spores.

\subsubsection{Sapburn}
They appears as dark brown spots or blotches that are often slightly sunken. 
Likewise, damage can occur as runs or streaks down the cheek or as scattered marks around the stem and shoulder, resulting from sap exposure. 

\subsubsection{Skin Browning}
It may take two forms. First form is abrasion while the second form is sap browning. 
...
These types of browning are generally limited to the skin and do not penetrate deeply.
...
Furthermore, these defects are usually cosmetic in nature and do not significantly affect the flesh.
...
For the computer vision method of getting the length and width of the mango a foreground masking is generated by getting absolute difference
...
Likewise, the pretrained Faster RCNN model used is the MobileNetV3.
