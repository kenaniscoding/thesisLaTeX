...
\subsubsection{Calibration by Getting the Pixels per cm}
To get the accurate length and width of a mango the pixel-to-centimeter ratio of an image using a reference object, 
such as a coin, with a known real-world size.  As seen on Listing~\ref{lst:pxpercm}, 
it takes the bounding box 
coordinates of the reference object (which is a Philippine 1 peso coin)
and computes its width and height in pixels. Since the reference object may not 
be perfectly square, the larger of these two values is selected as the effective reference size (of the coin) 
in pixels. Using the 
known physical size of the coin in cm, the function then calculates how many pixels correspond to one 
centimeter. This means that if a coin of known diameter appears in the image, the function determines the relationship 
between its pixel size and real-world measurement to establish a scale. Once this scale is set, the
mango image can be measured accurately by comparing its pixel size to the calibrated ratio.

\begin{lstlisting}[
float=h,
caption=Calibration of Reference Philippine Coin, 
label=lst:pxpercm,
language=TeX,
frame=single]
def calibrate_with_reference_object(self, image, reference_box, reference_size_cm):
    ...
    # Use the max dimension for calibration
